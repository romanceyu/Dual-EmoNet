...
class GradCAMGenerator:
    ...
    def _load_model(self, model_dir, model_file):
        """加载预训练模型 (含完整性校验)"""
        from approach.DA_EmoNet2 import DA_EmoNet  # 假设模型定义在此处
    ...
    def generate_cam(self, image_path):
        ...
        try:
            ...
            # 前向传播，捕获模型输出
            output, attention_map1, attention_map2, attention_map3 = self.model(img_tensor)
            probabilities = torch.nn.functional.softmax(output, dim=1)
            pred_class = torch.argmax(probabilities).item()

            # 使用 attention_map3 作为热力图
            cam = attention_map3.squeeze().cpu().detach().numpy()
    ...
    def visualize(self, original_img, cam, output_path, filename):
        ...
        try:
            ...
            # 创建叠加图像
            heatmap = cv2.applyColorMap(np.uint8(255 * blurred_cam), cv2.COLORMAP_VIRIDIS)
            ...
            axes[1].set_title('(b) Attention Map', fontsize=14, pad=10)
            axes[1].axis('off')
            ...
            # 保存结果
            output_file = os.path.join(output_path, f"{filename}_attention.png")
...
def main():
    ...
    OUTPUT_DIR = "/root/autodl-tmp/ResEmoteNet/attention/results5"
    try:
        # 初始化生成器
