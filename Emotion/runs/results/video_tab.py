...
class VideoTab(QWidget):
    ...
    def initUI(self):
        ...
        top_layout.addWidget(QLabel("视频源:"))
        ...
        top_layout.addWidget(self.start_stop_btn)
        ...
        if not self.recording:
            ...
            try:
                ...
                file_path, _ = QFileDialog.getSaveFileName(
                    self, "保存视频", f"emotion_video_{timestamp}.avi",
                    "AVI视频 (*.avi);;所有文件 (*)", options=options)
                ...
                if self.current_frame is not None:
                    ...
                    self.output_video = cv2.VideoWriter(file_path, fourcc, 20.0, (width, height))

                    self.recording = True
                    self.record_btn.setText("停止录制")
                    self.parent().statusBar().showMessage("录制中...", 0)  # 0表示不会自动消失
                else:
            ...
            self.parent().statusBar().showMessage("录制已停止", 3000)
    ...
    def exportResults(self):
        ...
        try:
            ...
            # 显示成功消息
            self.parent().statusBar().showMessage(f"结果已导出到：{file_path}", 3000)
