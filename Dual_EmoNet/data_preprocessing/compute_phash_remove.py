...
def find_similar_images_and_filter(train_dir, test_dir, val_dir, output_dir, threshold=5, remove_ratio=0.90):
    """
    1. 计算 `train`, `test`, `val` 数据集的哈希值
    2. 识别 `test` 和 `train` 的相似图片，并 50% 概率删除 `test` 里的重复图片
    3. 复制处理后的数据集到 `output_dir`
    """
    ...
    # 找到 `test` 和 `train` 之间的重复项
    duplicate_test_images = set()
        ...
        for tr_hash, tr_files in train_hashes.items():
            if t_hash - tr_hash <= threshold:
                for t_path in t_files:
                    if random.random() < remove_ratio:  # 50% 概率删除
                        duplicate_test_images.add(t_path)
                        deleted_test_count += 1

    # 找到 `val` 和 `train` 之间的重复项
    ...
    print(f"\n找到的 `test` 重复图片数量: {len(duplicate_test_images)}")
    print(f"找到的 `val` 重复图片数量: {len(duplicate_val_images)}")
    print(f"\n实际删除 `test` 图片数量: {deleted_test_count}")
    print(f"实际删除 `val` 图片数量: {deleted_val_count}")

    # 复制数据集到新的目录
    for dataset_name, dataset_dir, duplicate_set in [
        ("train", train_dir, set()),  # 训练集不变
        ("test", test_dir, duplicate_test_images),
        ("val", val_dir, duplicate_val_images),
    ]:
...
if __name__ == "__main__":
    ...
    # 输出数据集路径
    output_dir = "/root/autodl-tmp/ResEmoteNet/rafdb_90"

    # 运行去重并重新划分数据集
    find_similar_images_and_filter(train_dir, test_dir, val_dir, output_dir)
